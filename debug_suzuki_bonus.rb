...
puts "🔍 Debugging 鈴木愛美's bonus calculation..."
...
puts "👤 User: #{suzuki.name}"
puts "📊 Level: #{suzuki.level&.name}"
puts "🎯 Bonus eligible: #{suzuki.bonus_eligible?}"
...
# 自分の購入データ
my_purchases = suzuki.purchases.where(purchased_at: start_date..end_date)
puts "\n🛒 My purchases: #{my_purchases.count}"

my_total_bonus = 0
my_purchases.each do |purchase|
  purchase.purchase_items.each do |item|
    bonus = suzuki.bonus_for_purchase_item(item)
...
# 子孫の購入データ
descendant_ids = suzuki.descendant_ids
...
# 直下無資格者
direct_non_eligible = suzuki.referrals.reject(&:bonus_eligible?)
...
# 実際の子孫計算対象
filtered_descendant_ids = descendant_ids.reject { |uid| uid == suzuki.id || direct_non_eligible_ids.include?(uid) }
...
filtered_descendant_purchases.each do |purchase|
  purchase.purchase_items.each do |item|
    ...
    suzuki_level_at_purchase = suzuki.level_at(purchase.purchased_at)
    ...
    if purchase_user_price > suzuki_price
      diff = purchase_user_price - suzuki_price
      bonus = diff * item.quantity
      descendant_total_bonus += bonus
      puts "  - #{purchase.user.name}: #{item.product.name} x#{item.quantity}"
      puts "    Purchase price: ¥#{purchase_user_price}, Suzuki price: ¥#{suzuki_price}"
...
# 直下無資格者の購入データ
referrals = suzuki.referrals.reject(&:bonus_eligible?)
puts "\n👥 Direct non-eligible referrals: #{referrals.count}"
referrals.each do |ref|
  puts "  - #{ref.name} (#{ref.level&.name})"
end

referral_total_bonus = 0
referrals.each do |child|
  ...
  child_purchases.each do |purchase|
    purchase.purchase_items.each do |item|
      ...
      suzuki_price = product.product_prices.find_by(level_id: suzuki_level_at_purchase.id)&.price || 0
      diff = base_price - suzuki_price
      if diff.positive?
        bonus = diff * item.quantity
        referral_total_bonus += bonus
        puts "  - #{child.name}: #{item.product.name} x#{item.quantity}"
        puts "    Base price: ¥#{base_price}, Suzuki price: ¥#{suzuki_price}"
        puts "    Bonus: ¥#{bonus.to_i}"
      end
    end
  end
...
actual_bonus = suzuki.bonus_in_period(start_date, end_date)
...
puts "Expected: ¥290,000"
puts "Difference: ¥#{(actual_bonus - 290000).to_i}"